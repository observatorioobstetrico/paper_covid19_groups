...
odds_function <- function(dat, met = "midp") {
  ...
  or_fit <- oddsratio(matriz, correction=TRUE, method = met, conf.level = 1-round(0.05/3,4))
  return(or_fit)
}
...
or_rl <- function(tab){
  ...
  out[1,1] <- exp(tab[2,1])
  out[1,2] <- exp(tab[2,1]-2.39*tab[2,2])
  out[1,3] <- exp(tab[2,1]+2.39*tab[2,2])  
  out[2,1] <- exp(tab[3,1])
  out[2,2] <- exp(tab[3,1]-2.39*tab[3,2])
  out[2,3] <- exp(tab[3,1]+2.39*tab[3,2])  
  dimnames(out) <- list("Groups" = c("preg", "puerp"), 
                           "Measures" = c("OR", "L 98.33%", "U 98.33%"))
  return(out)
}
...
or_rlo_inv_nao <- function(coef, ep){
  ...
  out[1,2] <- exp(coef[1,2]-2.39*ep[1,2])
  out[1,3] <- exp(coef[1,2]+2.39*ep[1,2])  
  out[2,1] <- exp(coef[1,3])
  out[2,2] <- exp(coef[1,3]-2.39*ep[1,3])
  out[2,3] <- exp(coef[1,3]+2.39*ep[1,3]) 
  dimnames(out) <- list("Groups" = c("preg", "puerp"), 
                        "Measures" = c("OR", "L 98.33%", "U 98.33%"))
  return(out)
}
...
or_rlo_ninv_nao <- function(coef, ep){
  ...
  out[1,2] <- exp(coef[2,2]-2.39*ep[2,2])
  out[1,3] <- exp(coef[2,2]+2.39*ep[2,2])  
  out[2,1] <- exp(coef[2,3])
  out[2,2] <- exp(coef[2,3]-2.39*ep[2,3])
  out[2,3] <- exp(coef[2,3]+2.39*ep[2,3]) 
  dimnames(out) <- list("Groups" = c("preg", "puerp"), 
                        "Measures" = c("OR", "L 98.33%", "U 98.33%"))
  return(out)
}
